...
import getListingByUnits from "@/services/listing/metrics/listing-by-units";

export function ListingByUnits() {
	const [listings, setListings] = React.useState<Record<string, number>>({});

	const { isFetching, isError, error } = useQuery(["listing-by-units"], () => getListingByUnits(), {
		onSuccess(data) {
			setListings(data);
		},
	});
	...
	return (
		<ErrorBoundary>
			...
			>
				<Card className="">
					<CardHeader>
						<CardTitle>Listing By Units</CardTitle>
